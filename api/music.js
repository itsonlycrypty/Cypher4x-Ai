export default async function handler(req, res) {
  ...
  try {
    // Try Tunova's REST endpoint (not MCP)
    const body = {
      prompt: prompt || 'cinematic music',
      make_instrumental: false,
      // Some providers accept duration, some don't. Send anyway.
    ...
    const r = await fetch('https://api.tunova.ai/v1/music/generate', {
      ...
      headers: {
        ...
        'Authorization': `Bearer ${TUNOVA_API_KEY}`,
    ...
    if (!r.ok) return res.status(r.status).json({
      error: `Tunova HTTP ${r.status}`,
      detail: rawText.slice(0, 500),
      hint: 'Confirm endpoint at https://api.tunova.ai/docs',
    })
