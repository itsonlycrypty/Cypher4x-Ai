...
// ==============================================
// ⚡ CYPHER4X v19.0 — ENHANCED EDITION
// ==============================================
...
export default function App() {
  ...
  const [aiChat, setAiChat] = useState([]) // 🤖 SEPARATE AI CHAT
  const [userChat, setUserChat] = useState([]) // 💬 USER-TO-USER CHAT
  ...
  // ===== SEND MESSAGE — DEPENDING ON ACTIVE TAB =====
  ...
  if (isBooting) {
    return (
      <div style={{
        ...
        {/* ASCII LOGO */}
        <pre style={{ 
          color: '#ff003c', fontSize: '6px', marginBottom: '30px', 
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden', position: 'relative' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', width: '100%', height: '100%' }}>
          ...
          <div style={{ flex: 1, padding: '16px', overflowY: 'auto', overflowX: 'hidden', minHeight: 0 }}>
            ...
            {/* ============================================== */}
            {/* 🤖 CYPHER4X AI SECTION — DISTINCT DESIGN */}
            {/* ============================================== */}
            {activeTab === "ai" && (
              <>
                <div style={{ 
                ...
                {isLoading && (
                  <div style={{ 
                    padding: '16px', color: '#ff003c', fontSize: '15px', 
                    border: '2px solid #ff003c', borderRadius: '8px',
                    backgroundColor: 'rgba(255,0,60,0.06)', display: 'flex', alignItems: 'center', gap: '10px'
                  }}>
                    <span style={{ animation: 'pulse 1s infinite' }}>☠️</span>
                    <span>Processing — UNRESTRICTED...</span>
            ...
            {/* ============================================== */}
            {/* 💬 USER-TO-USER CHAT — COMPLETELY DIFFERENT DESIGN */}
            {/* ============================================== */}
            {activeTab === "userChat" && (
              <>
                <div style={{ 
                  ...
                  💬 PUBLIC USER CHAT — Connect & Chat with other CYPHER4X users • NO AI here
                </div>
                
                {userChat.length === 0 ? (
                  <div style={{ textAlign: 'center', color: '#888', marginTop: '80px' }}>
                    <p style={{ fontSize: '48px', marginBottom: '16px' }}>💬</p>
                    <p style={{ fontSize: '22px', color: '#4ade80', fontWeight: 'bold', marginBottom: '8px' }}>NO MESSAGES YET</p>
                    <p style={{ color: '#888', fontSize: '16px', marginBottom: '24px' }}>Be the first to say hello! Connect with the community</p>
                    <div style={{ 
                      maxWidth: '400px', margin: '0 auto', textAlign: 'left', 
                      backgroundColor: '#111', padding: '16px', borderRadius: '8px',
                      border: '1px solid #333'
                    }}>
                      <p style={{ color: '#4ade80', fontSize: '13px', margin: '0 0 8px', fontWeight: 'bold' }}>👋 Chat with others:</p>
                      <p style={{ color: '#aaa', fontSize: '13px', margin: '4px 0' }}>• Introduce yourself</p>
                      <p style={{ color: '#aaa', fontSize: '13px', margin: '4px 0' }}>• Share ideas & projects</p>
                      <p style={{ color: '#aaa', fontSize: '13px', margin: '4px 0' }}>• Help each other out</p>
                    </div>
                  </div>
                ) : (
                  userChat.map((msg) => (
                    <div
                      key={msg.id}
                      style={{
                        margin: '10px 0', padding: '14px', borderRadius: '12px',
                      ...
                      <div style={{ fontSize: '12px', color: '#888', marginBottom: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          <div style={{
                            ...
                            overflow: 'hidden', backgroundColor: '#222', border: '2px solid #4ade80'
                          }}>
                            {msg.avatar ? (
                              <img src={msg.avatar} alt="Avatar" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                            ) : (
                          ...
                          <span style={{ fontSize: '11px', color: '#555' }}>@{msg.username}</span>
                        ...
                        <div style={{ display: 'flex', gap: '4px' }}>
                          {msg.username === profile.username && (
                            <button onClick={() => startEdit(msg, "userChat")} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '16px', padding: '2px 6px' }} title="Edit">✏️</button>
                          )}
                          {(msg.username === profile.username || isAdmin) && (
                            <button onClick={() => deleteMessage(msg.id, "userChat")} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '16px', padding: '2px 6px' }} title="Delete">🗑️</button>
                          )}
                        </div>
                      </div>
                      ...
                      {editingMsg?.id === msg.id ? (
                        <div>
                          <textarea
                            ...
                            style={{
                              width: '100%', padding: '12px', backgroundColor: '#000', 
                              border: '2px solid #4ade80', color: '#fff', borderRadius
