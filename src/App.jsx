        ...
        <>
          ...
          <div style={styles.sidebar}>
            ...
            <div style={styles.sidebarSection}>
              ...
              <div style={styles.statsCard}>
                <div style={styles.statRow}>
                  <span style={styles.statLabel}><Icon name="hourglass" size={14} color="#888" /> Uptime</span>
                  <span style={styles.statValue}>{formatUptime(stats.uptime)}</span>
                </div>
                <div style={styles.statRow}>
                  <span style={styles.statLabel}><Icon name="rocket" size={14} color="#888" /> Runtime</span>
                  <span style={styles.statValue}>{stats.runtime}</span>
                </div>
                <div style={styles.statRow}>
                  <span style={styles.statLabel}><Icon name="chat" size={14} color="#888" /> General Chat</span>
                  <span style={styles.statValue}>{generalMessages.length}</span>
                </div>
                <div style={styles.statRow}>
                  <span style={styles.statLabel}><Icon name="megaphone" size={14} color="#888" /> Announcements</span>
                  <span style={styles.statValue}>{announcements.length}</span>
                </div>
                <div style={styles.statRow}>
                  <span style={styles.statLabel}><Icon name="bot" size={14} color="#888" /> AI Responses</span>
                  <span style={styles.statValue}>{aiMessages.length}</span>
                </div>
                <div style={styles.statRow}>
                  <span style={styles.statLabel}><Icon name="settings" size={14} color="#888" /> Version</span>
                  <span style={styles.statValue}>{VERSION}</span>
                </div>
                <div style={styles.statRow}>
                  <span style={styles.statLabel}><Icon name="brain" size={14} color="#888" /> Model</span>
                  <span style={styles.statValue}>gpt-oss-120b</span>
                </div>
              </div>
            </div>
              ...
              <div style={styles.profileCardSidebar}>
                ...
                <div style={styles.profileInfo}>
                  ...
                  {profile?.username === ADMIN_USERNAME && (
                    <span style={styles.adminBadge}><Icon name="crown" size={12} color="#fff" /> ADMIN</span>
                  )}
                </div>
              </div>
            ...
            <div style={styles.sidebarSection}>
              ...
              >
                <Icon name="refresh" size={14} color="#fff" /> Reset Default Prompt
              </button>
            </div>
                    ...
                    <>
                      <div style={styles.msgSender}>
                        ...
                        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                          ...
                          <button onClick={() => deleteMessage(msg.id, "ai")} style={styles.msgDeleteBtn}><Icon name="trash" size={14} color="#ff4444" /></button>
                        </div>
                      </div>
                      <div style={styles.msgText}>{msg.content}</div>
                      {msg.role === "user" && (
                        <div style={styles.msgActions}>
                          <button onClick={() => startEditMessage(msg, "ai")} style={styles.msgEditBtn}><Icon name="edit" size={14} color="#ff6688" /> Edit</button>
                        </div>
                      )}
                    </>
                  )}
        ...
        {activeTab === "general" && (
          <>
            ...
            {generalMessages.length === 0 ? (
              <div style={styles.emptyState}>
                <h2 style={styles.heading}><Icon name="chat" size={28} color="#ff003c" /> GENERAL CHAT</h2>
                      ...
                      <div style={styles.msgSender}>
                        <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                          ...
                          <span style={{ fontWeight: msg.isAdmin ? 'bold' : 'normal', color: msg.isAdmin ? '#ff003c' : '#ddd' }}>
                            {msg.sender} {msg.isAdmin && <Icon name="crown" size={12} color="#ff003c" />}
                          </span>
                          {msg.edited && <span style={styles.editedTag}>(edited)</span>}
                        </span>
                        {msg.username === profile?.username && (
                          <div style={{ display: 'flex', gap: '6px' }}>
                            <button onClick={() => startEditMessage(msg, "general")} style={styles.msgEditBtn}><Icon name="edit" size={14} color="#ff6688" /></button>
                            <button onClick={() => deleteMessage(msg.id, "general")} style={styles.msgDeleteBtn}><Icon name="trash" size={14} color="#ff4444" /></button>
                          </div>
                        )}
                      </div>
                      <div style={styles.msgText}>{msg.content}</div>
        ...
        {activeTab === "announcements" && (
          <>
            ...
            {announcements.length === 0 ? (
              <div style={styles.emptyState}>
                <h2 style={styles.heading}><Icon name="megaphone" size={28} color="#ff003c" /> NO ANNOUNCEMENTS YET</h2>
                    ...
                    <>
                      <div style={styles.msgSender}>
                        <span style={{ display: 'flex', alignItems: 'center', gap: '6px', fontWeight: 'bold', color: '#ff003c' }}>
                          <Icon name="megaphone" size={14} color="#ff003c" /> {msg.sender}
                        </span>
                        {profile?.username === ADMIN_USERNAME && (
                          <div style={{ display: 'flex', gap: '6px' }}>
                            <button onClick={() => startEditMessage(msg, "announcements")} style={styles.msgEditBtn}><Icon name="edit" size={14} color="#ff6688" /></button>
                            <button onClick={() => deleteMessage(msg.id, "announcements")} style={styles.msgDeleteBtn}><Icon name="trash" size={14} color="#ff4444" /></button>
                          </div>
                        )}
                      </div>
                      <div style={styles.msgText}>{msg.content}</div>
