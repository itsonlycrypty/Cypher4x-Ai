...
export default function App() {
    ...
    try {
      ...
      const poll = setInterval(async () => {
        ...
        try {
          ...
          if (url) { clearInterval(poll); setMusicDownloadUrl(url); setMusicInfo('✓ Ready!'); const audio = new Audio(url); audio.volume = 0.9; audio.play().catch(()=>{}); setMusicAudioRef(audio); setMusicPlaying(true); audio.onended = () => setMusicPlaying(false); setMusicGenerating(false) }
          else if (status.status === 'failed' || status.error) { clearInterval(poll); setMusicInfo('⚠️ Tunova failed — using built-in'); fallback() }
          else { setMusicInfo(`⏳ Generating... (${attempts * 3}s)`) }
        } catch (e) { if (attempts >= 20) { clearInterval(poll); fallback() } }
        if (attempts >= 20) { clearInterval(poll); fallback() }
      }, 3000)
    } catch (e) { fallback() }
  }
  ...
  const handleGenerateVideo = async () => {
    ...
    if (!videoDesc.trim()) { alert('Describe the video first.'); return }
    ...
    const fallback = () => {
      ...
      setVideoInfo(`🎬 Built-in engine — ${style.label} • ${videoDuration}s`)
      ...
      setTimeout(() => {
        ...
        setTimeout(async () => {
          ...
          setVideoResultUrl(URL.createObjectURL(blob)); setVideoInfo('✓ Video ready! Tap download.')
        }, videoDuration * 1000)
      }, 100)
    }
    try {
      ...
      const data = await res.json()
      if (!res.ok || data.error) { setVideoInfo('⚠️ Agnes unavailable — built-in engine'); return fallback() }
      const vId = data.video_id || data.id || data.task_id || data.data?.id
      if (!vId) { setVideoInfo('⚠️ No task ID — built-in engine'); return fallback() }
      ...
      const poll = setInterval(async () => {
        ...
        try {
          ...
          if (url) { clearInterval(poll); setVideoResultUrl(url); setVideoInfo('✓ Ready!'); setVideoPolling(false) }
          else if (status.status === 'failed' || status.error) { clearInterval(poll); setVideoInfo('⚠️ Agnes failed — built-in'); setVideoPolling(false); fallback() }
          else { setVideoInfo(`⏳ Generating... (${attempts * 5}s)`) }
        } catch (e) { if (attempts >= 15) { clearInterval(poll); setVideoPolling(false); fallback() } }
        if (attempts >= 15) { clearInterval(poll); setVideoPolling(false); fallback() }
      }, 5000)
    } catch (e) { fallback() }
  }
  const handleStopLocalVideo = async () => { if (!videoRecorderRef.current) return; const blob = await videoRecorderRef.current.stop(); videoRecorderRef.current = null; setVideoRecording(false); setVideoResultUrl(URL.createObjectURL(blob)); setVideoInfo('✓ Video ready!') }
  ...
  const runCyberCommand = async (cmd) => {
    ...
    try {
      switch (base) {
        ...
        case 'ping': out = arg1 ? `PING ${arg1}: 4 packets, 0% loss` : 'Usage: ping <host>'; break
        case 'ifconfig': out = `wlan0: inet 192.168.1.${Math.floor(Math.random() * 200) + 10}`; break
        case 'curl': { if (!arg1) { out = 'Usage: curl <url>'; break } try { const r = await fetch(arg1); out = `HTTP ${r.status}\n\n${(await r.text()).slice(0, 500)}` } catch (e) { out = 'curl: ' + e.message } break }
        case 'dns': { if (!arg1) { out = 'Usage: dns <domain>'; break } try { const r = await fetch(`https://dns.google/resolve?name=${encodeURIComponent(arg1)}&type=A`); const d = await r.json(); out = d.Answer ? d.Answer.map(a => `A\t${a.data}`).join('\n') : 'No records' } catch (e) { out = 'Error: ' + e.message } break }
        case 'ip': { if (!arg1) { out = 'Usage: ip <address>'; break } try { const r = await fetch(`https://ipapi.co/${encodeURIComponent(arg1)}/json/`); const d = await r.json(); out = `IP: ${d.ip}\nCity: ${d.city}\nCountry: ${d.country_name}` } catch (e) { out = 'Error: ' + e.message } break }
        case 'portscan': { if (!arg1) { out = 'Usage: portscan <host>'; break } out = `⚠️ SIM\nTarget: ${arg1}\n`; [21,22,80,443,3306,8080].forEach(p => { out += `  ${p}: ${Math.random() > 0.75 ? 'open' : 'closed'}\n` }); break }
        ...
        case 'passcheck': { if (!args) { out = 'Usage: passcheck <password>'; break } const c = { len: args.length >= 12, low: /[a-z]/.test(args), up: /[A-Z]/.test(args), dig: /\d/.test(args), sp: /[^A-Za-z0-9]/.test(args) }; const s = Object.values(c).filter(Boolean).length; out = `Score ${s}/5 — ${s >= 4 ? '🟢 Strong' : s >= 3 ? '🟡 Medium' : '🔴 Weak'}`; break }
  ...
  const runCyberTool = async () => {
    ...
    try {
      switch (cyberTool) {
        ...
        case 'dns': { const r = await fetch(`https://dns.google/resolve?name=${encodeURIComponent(cyberToolInput)}&type=A`); const d = await r.json(); out = d.Answer ? d.Answer.map(a => `A  ${a.data}`).join('\n') : 'No records'; break }
    ...
    <div style={{...styles.practicalContainer, backgroundColor: practicalBg, color: practicalBg === '#ffffff' ? '#000000' : '#ffffff'}}>
      ...
      <div style={styles.practicalCanvas}>
        {practicalLogs.map(log => {
          ...
          return (
            <div key={log.id} style={{...styles.practicalMsg, alignSelf: isUser ? 'flex-end' : 'flex-start', backgroundColor: bubbleBg, color: bubbleColor, position: 'relative'}}>
              <div style={styles.practicalMsgActions}>
                {isUser && (
                  <button onClick={() => handlePracticalEdit(log)} style={styles.practicalMsgActionBtn} title="Edit">
                    <Icon name="edit" size={12} color={isUser ? '#000' : '#666'} />
                  </button>
                )}
                <button onClick={() => handlePracticalDelete(log.id)} style={styles.practicalMsgActionBtn} title="Delete">
                  <Icon name="trash" size={12} color={isUser ? '#000' : '#666'} />
                </button>
              </div>
                ...
                <div style={{width: '100%'}}>
                  {log.content.type === 'image' ? (
                    <>
                      ...
                      <div style={styles.practicalArtifactActions}>
                        <button onClick={() => handleArtifactDownload(log.content)} style={styles.practicalArtifactBtnPrimary}><Icon name="download" size={14} color="#000" /> DOWNLOAD</button>
                        <button onClick={() => openArtifactInTab(log.content)} style={styles.practicalArtifactBtnSecondary}>OPEN IN TAB</button>
                      </div>
                    </>
                  ) : log.content.type === 'code' ? (
                    <>
                      <div style={{marginBottom: 8, fontSize: 12, color: bubbleColor, opacity: 0.7}}>
                        📄 {log.content.title} · <span style={{color: '#00ff41'}}>{log.content.lang}</span>
                      </div>
                    ...
                    <>
                      <div style={styles.practicalPreviewHeader}>
                        <span style={{color: '#00ff41', fontWeight: 'bold', fontSize: 12}}>🎨 {log.content.title}</span>
                        <span style={{color: bubbleColor, opacity: 0.6, fontSize: 10, letterSpacing: 1}}>{log.content.type.toUpperCase()}</span>
                      </div>
                      <iframe
                        srcDoc={log.content.html}
                        style={styles.practicalPreviewIframe}
        ...
        {practicalProcessing && (
          <div style={{color: '#00ff41', fontStyle: 'italic', display: 'flex', alignItems: 'center', gap: 8, padding: 12}}>
            ...
            <span>{practicalActionText || 'AI is building...'}</span>
          </div>
        )}
      ...
      <form onSubmit={handlePracticalSubmit} style={{...styles.practicalInputRow, backgroundColor: practicalBg === '#ffffff' ? '#fff' : '#0a0a0a', borderTopColor: practicalBg === '#ffffff' ? '#ddd' : '#333'}}>
        <input
          ...
          placeholder="Tell me what to build — try 'snake game', 'calculator', 'password tool', or 'show me a sunset'"
    ...
    <div style={styles.workspaceContainer}>
      ...
      <div style={styles.workspaceBody}>
        <div style={styles.workspaceLeftPanel}>
          <div style={styles.workspacePanelHeader}><Icon name="layers" size={16} color={theme.primary} /> TASKS ({workspaceTasks.length})</div>
            ...
            {workspaceTasks.map(t => (
              <div key={t.id} style={styles.workspaceTaskItem}>
                ...
                <div style={{width: '100%', height: 4, backgroundColor: '#111', borderRadius: 2, overflow: 'hidden'}}>
                  <div style={{width: `${t.progress}%`, height: '100%', backgroundColor: t.status === 'done' ? '#00ff41' : theme.primary}} />
                </div>
              </div>
            ))}
          </div>
          <div style={{...styles.workspacePanelHeader, marginTop: 15}}><Icon name="terminal" size={16} color={theme.primary} /> LOGS</div>
        ...
        <div style={styles.workspaceCenterPanel}>
          ...
          <div style={styles.workspaceCanvasArea}>
            {workspaceActiveTab === 'canvas' && (workspaceContent?.type === '3d' ? <div style={{textAlign: 'center'}}><h3 style={{color: theme.primary, marginBottom: 20}}>{workspaceContent.data.title}</h3><img src={workspaceContent.data.image} alt="Generated" style={{maxWidth: '100%', maxHeight: '60vh', borderRadius: 12}} /></div> : <div style={{textAlign: 'center', color: '#555'}}><Icon name="image" size={64} color="#333" /><p style={{marginTop: 15}}>No visual output yet.</p></div>)}
        ...
        <div style={styles.workspaceRightPanel}>
          <div style={styles.workspacePanelHeader}><Icon name="sparkles" size={16} color={theme.primary} /> COMMAND</div>
    ...
    <div style={styles.settingsFullscreen}>
      ...
      <div style={styles.settingsBodyFull}>
        ...
        <div style={styles.settingsSection}>
          ...
          <div style={styles.settingItem}><span>Secondary color</span><input type="color" value={theme.secondary} onChange={(e) => setTheme({ ...theme, secondary: e.target.value })} style={styles.colorPicker} /></div>
        ...
        <div style={styles.settingsSection}>
          ...
          <div style={styles.settingItem}><span>🔒 Restrict Tools</span><label className="toggle-switch"><input type="checkbox" checked={settings.restrictTools} onChange={(e) => setSettings({ ...settings, restrictTools: e.target.checked })} /><span className="toggle-slider"></span></label></div>
        </div>
        ...
        <div style={styles.settingsSection}>
          <h3 style={{ ...styles.settingsSectionTitle, color: theme.primary }}>Background Image</h3>
  ...
  if (showVideoPanel) return (
    <div style={styles.settingsFullscreen}>
      ...
      <div style={styles.settingsBodyFull}>
        <div style={styles.settingsSection}><h3 style={{ ...styles.settingsSectionTitle, color: theme.primary }}>Describe Your Video</h3><input value={videoDesc} onChange={(e) => setVideoDesc(e.target.value)} placeholder="anime sakura, cinematic drone..." style={styles.settingsSelect} /><p style={styles.bgHint}>Tries Agnes AI → falls back to built-in engine.</p></div>
  ...
  if (showChatOverview) return (
    <div style={{...styles.chatOverviewContainer, backgroundColor: theme.secondary, backgroundImage: backgroundImage ? `url(${backgroundImage})` : 'none', backgroundSize: 'cover', backgroundPosition: 'center'}}>
      ...
      <div style={styles.chatOverviewInputRowRaised}>
        <input type="text" value={chatOverviewInput} onChange={(e) => setChatOverviewInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && sendOverviewText()} placeholder={pendingCode ? 'Answer...' : replyingTo ? 'Reply...' : 'Type a message...'} style={styles.chatOverviewInput} disabled={isProcessing} />
      ...
      {sidebarOpen && (<>
        ...
        <div style={styles.sidebar}>
          ...
          <div style={styles.sidebarSection}>
            <h3 style={{...styles.sectionTitle, color: theme.primary}}><Icon name="calendar" size={16} color={theme.primary} /> DASHBOARD</h3>
          ...
          <div style={styles.sidebarSection}>
            <h3 style={{...styles.sectionTitle, color: theme.primary}}><Icon name="desktop" size={16} color={theme.primary} /> VIEW MODE</h3>
          ...
          <div style={styles.sidebarSection}>
            <h3 style={{...styles.sectionTitle, color: theme.primary}}><Icon name="sparkles" size={16} color={theme.primary} /> QUICK TOOLS</h3>
          ...
          <div style={styles.sidebarSection}>
            <h3 style={{...styles.sectionTitle, color: theme.primary}}><Icon name="chart" size={16} color={theme.primary} /> SYSTEM</h3>
          ...
          <div style={styles.sidebarSection}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
              <h3 style={{...styles.sectionTitle, color: theme.primary}}><Icon name="chat" size={16} color={theme.primary} /> CHAT ({chats.length})</h3>
          ...
          <div style={styles.sidebarSection}>
            <h3 style={{...styles.sectionTitle, color: theme.primary}}><Icon name="user" size={16} color={theme.primary} /> PROFILE</h3>
          ...
          <div style={styles.sidebarSection}>
            <h3 style={{...styles.sectionTitle, color: theme.primary}}><Icon name="alertTriangle" size={16} color={theme.primary} /> DANGER ZONE</h3>
    ...
    <div style={{ ...styles.appPC, backgroundColor: theme.secondary }}>
      <header style={{...styles.headerPC, borderBottomColor: hexA(theme.primary, 0.3)}}>
        ...
        <div style={styles.headerRight}>
          ...
          <button onClick={() => { setShowPracticalWorkspace(true) }} style={styles.settingsBtnPC} title="Practical Workspace"><Icon name="zap" size={20} color="#00ff41" /></button>
...
const styles = {
  ...
  practicalMsgActions: { position: 'absolute', top: 4, right: 4, display: 'flex', gap: 2, zIndex: 2, opacity: 0.75 },
  practicalMsgActionBtn: { background: 'rgba(0,0,0,0.15)', border: 'none', padding: 4, borderRadius: 4, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', minWidth: 22, minHeight: 22 },
  practicalPreviewHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 10px', backgroundColor: 'rgba(0,0,0,0.25)', borderTopLeftRadius: 8, borderTopRightRadius: 8, marginBottom: 0 },
  practicalPreviewIframe: { width: '100%', height: 380, border: '1px solid #333', borderTop: 'none', borderBottomLeftRadius: 8, borderBottomRightRadius: 8, backgroundColor: '#fff', display: 'block' },
  practicalArtifactActions: { display: 'flex', gap: 8, marginTop: 10, flexWrap: 'wrap' },
  practicalArtifactBtnPrimary: { display: 'flex', alignItems: 'center', gap: 6, padding: '8px 16px', backgroundColor: '#00ff41', color: '#000', border: 'none', borderRadius: 6, fontWeight: 'bold', cursor: 'pointer', fontSize: 12 },
  practicalArtifactBtnSecondary: { padding: '8px 16px', backgroundColor: 'transparent', color: 'inherit', border: '1px solid currentColor', borderRadius: 6, fontWeight: 'bold', cursor: 'pointer', fontSize: 12, opacity: 0.8 },
  ...
  settingsBodyFull: { flex: 1, minHeight: 0, overflowY: 'auto', padding: 20, display: 'flex', flexDirection: 'column', gap: 20 },
  ...
  bgHint: { color: '#888', fontSize: 12, marginTop: 8, fontStyle: 'italic' },
  ...
  chatOverviewMessages: { flex: 1, minHeight: 0, overflowY: 'auto', padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 8 },
