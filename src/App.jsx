    ...
    if (this.state.hasError) {
      return (
        <div style={{
          ...
          <p style={{ color: '#ff6688' }}>Please refresh the page.</p>
          <pre style={{ fontSize: '12px', color: '#888', maxWidth: '80%', overflow: 'auto' }}>
...
// Restructured to match v17.0.9 architecture
...
function AppContent() {
  ...
  // ======================================
  // STATE — Ordered exactly like v17.0.9
  // ======================================
  ...
  const [activeTab, setActiveTab] = useState("ai")   // AI tab first
  ...
  const [cooldown, setCooldown] = useState(false)      // added to prevent rapid retries
  ...
  // ======================================
  // REFS — Ordered exactly like v17.0.9
  // ======================================
  const fileInputRef = useRef(null)
  const messagesEndRef = useRef(null)
  const chatAreaRef = useRef(null)
  const synthRef = useRef(typeof window !== "undefined" ? window.speechSynthesis : null)
  const msgCounter = useRef(0)   // for unique IDs
  ...
  // ======================================
  // EFFECTS & LIFECYCLE — Ordered like v17.0.9
  // ======================================
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (step < BOOT_STEPS.length - 1) {
        ...
        setTimeout(() => {
          ...
          const savedGeneral = localStorage.getItem("cypher4x_general_chat")
          ...
          if (savedGeneral) setGeneralMessages(JSON.parse(savedGeneral))
  ...
  useEffect(() => { localStorage.setItem("cypher4x_general_chat", JSON.stringify(generalMessages)) }, [generalMessages])
  ...
  const sendAiMessage = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 429) {
          throw new Error(`HTTP 429 – Too many requests. Please wait a moment.`)
        }
      ...
      const errorContent = err.message.startsWith('HTTP 429') 
        ? `⚠️ RATE LIMITED: ${err.message} (cooldown 10s)` 
        : `⚠️ ERROR: ${err.message}`
      const errorMsg = {
        id: ++msgCounter.current,
        role: "assistant", 
      ...
      // If rate limited, activate cooldown to prevent spam
...
// ======================================
// STYLES — Red/Black Theme — COMPLETE
// ======================================
const styles = {
  // --- Layout & Containers ---
  app: { 
